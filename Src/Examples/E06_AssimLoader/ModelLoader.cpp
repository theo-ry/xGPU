...
namespace xgpu::assimp {
    ...
    bool model_loader::Load( xgpu::device& Device, std::string Filename ) noexcept
    {
        ...
        m_Directory = Filename.substr(0, Filename.find_last_of("/\\")); // Storing the file name
        m_Device    = Device;
            ...
            if (Props.mType == aiPTI_String)
            {
                ...
                else
                {
                    ...
                    switch(Props.mSemantic)
                    {
                        ...
                        {
                            if( xcore::string::FindStrI( pString->C_Str(), "_Base_Color" ) != -1 ) // This
                            {
                                Sampler.m_HintForType = "PBR_ALBEDO";
                            }
                            else if (xcore::string::FindStrI(pString->C_Str(), "_AO") != -1) // This
                            {
                                Sampler.m_HintForType = "PBR_OCCLUSION";
                            }
                            else if (xcore::string::FindStrI(pString->C_Str(), "_Normal") != -1) // This
                            {
                                Sampler.m_HintForType = "PBR_NORMAL";
                            }
                            else if (xcore::string::FindStrI(pString->C_Str(), "_Roughness") != -1) // This (Extra Credits)
                            {
